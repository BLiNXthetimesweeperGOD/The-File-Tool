...
class fileTools:
    ...
    def outputDirectory(path, dirname):
        """
        Takes a file path as input and generates/returns an output directory
        """
        outPath = os.getcwd()+f"/TEST/"
        if not os.path.exists(outPath):
            os.makedirs(outPath)
        return outPath
    
    def magicCheck(checkBytes, dataBytes): #For checking the "magic" (signature) of a file
        if checkBytes == dataBytes:
            return True
        else:
            return False
    ...
    def scanForBytesAndReturnEndOffset(path, string):
        """
        Reads an entire file into memory and checks it for a byte string
        
        Returns: True or False, offset to the end of the found bytes
        Format: scanForBytes(file path, byte string)
        """
        with open(path, 'rb') as file:
            content = file.read()
            offset = content.find(string)
            if offset != -1:
                # Return offset pointing to the end of the found string
                end_offset = offset + len(string)
...
class LE_BitReader:
    def __init__(self, file, chunk_size=1, offset=0):
        """Initialize LE_BitReader with a file object opened in binary mode ('rb')
...
class imageData:
    ...
    def indexedLinear(bits, file, offset, order, x, y):
        ...
        return convertedPixels[:pixelsNeeded]
    ...
    def bgr555LE(file, offset, alphaMode, x, y):
        ...
        with open(file, "rb") as pixels:
            ...
            pixelReader = LE_BitReader(pixels, chunk_size=2)
